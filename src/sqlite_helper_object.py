...
class SQLiteHelperObject:
    ...
    def insert_csv_data_into_sqlite_table(self, table_name, csv_fpath, *args,
                                          **kwargs):
        # read in csv file data
        header, data = csv_utils.read_in_data_as_tuples_float(csv_fpath)

        # create table if not exists yet
        column_names = SQLiteHelperObject.sanitize_column_names(
            header + list(kwargs.keys()))
        data_rows = [tuple(data_row + list(kwargs.values())) for data_row in
                     data]
        question_marks = ",".join(["?"] * len(column_names))
        self.create_table_if_not_exists(table_name, column_names)

        # insert the rows
        insert_cmd = "INSERT INTO {0} VALUES ({1})".format(table_name,
                                                           question_marks)
